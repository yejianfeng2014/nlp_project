...
## 一些配置参数
embed_size = 300  # 词向量的大小
max_features = 95000  # 多少唯一的单词使用
maxlen = 70  # 句子的最大长度
...
def load_and_prec():
    # train_df = pd.read_csv("../input_glove/trian_100.csv")
    # test_df = pd.read_csv("../input_glove/test_10.csv")
    train_df = pd.read_csv("../input_glove/train.csv")
    test_df = pd.read_csv("../input_glove/test.csv")
    ...
    tokenizer.fit_on_texts(list(train_X)) # 用训练集去训练这个分词器，生成id_word 和word_id 等信息 如果测试集没有对应的单词会丢弃（这个很严重。。。）
...
def load_glove(word_index):
    EMBEDDING_FILE = '../input_glove/glove.840B.300d.txt'
...
def load_fasttext(word_index):
    EMBEDDING_FILE = '../input_glove/wiki-news-300d-1M.vec'
...
#
# print(np.shape(embedding_matrix)) # (95000,300) 依然是300维度
#
#
